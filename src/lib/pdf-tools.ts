...
// Repair PDF - attempt to fix corrupted PDFs by re-rendering
...
export const repairPdf = async (
  ...
  try {
    // Try to load with pdf-lib first to detect issues
    try {
    ...
    // Load with pdfjs-dist which is more forgiving
...
// OCR PDF - extract text and make searchable
...
export const ocrPdf = async (
  ...
  try {
    for (let i = 1; i <= numPages; i++) {
      ...
      // Create new PDF page with image and invisible text layer
      const originalViewport = page.getViewport({ scale: 1 });
      const newPage = newPdfDoc.addPage([originalViewport.width, originalViewport.height]);
        ...
        for (const paragraph of block.paragraphs) {
          for (const line of paragraph.lines) {
            for (const word of line.words) {
              ...
              // Scale coordinates from OCR resolution to PDF coordinates
              const scaleRatio = originalViewport.width / viewport.width;
              const x = word.bbox.x0 * scaleRatio;
              // PDF coordinates are from bottom, OCR from top
